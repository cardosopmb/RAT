...
#####Reference: Cardoso, P., Fukushima, C.S. & Mammola, S. (subm.) Quantifying the international collaboration attitude of scholars.
#####Changed from v0.0.0:
#####Everything
...
wos <- function(id, user = NULL, pass = NULL, coll = c("SCI", "SSCI", "AHCI", "ISTP", "ISSHP","BSCI", "BHCI", "ESCI")){
  ...
  sid <- auth(username = user, password = pass)
  id <- wosr::pull_wos(query = query, editions = coll, sid = sid)
  return(id)
}
